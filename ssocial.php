...
<body>
    <main class="table" id="customers_table">
        <section class="table__header">
            <h1>Inventario del Seguro Social</h1>
    ...
    <form class="form" action="agregar.php" name="ejemplo" method="post">
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td><a href='eliminar.php?lote=" . urlencode($row["lote"]) . "' onclick='return confirm(\"¿Estás seguro de eliminar este registro?\")'>Eliminar</a></td>";
        echo "</tr>";
    }
} else {
...
?>
    ...
    <script>
        ...
        function exportToPDF() {
            ...
            let y = 10; // Coordenada y inicial para el contenido

            rows.forEach(row => {
                const cols = row.querySelectorAll('td, th');
                let x = 10; // Coordenada x inicial para el contenido
                cols.forEach(col => {
                    doc.text(col.innerText, x, y);
                    x += 30; // Espacio entre columnas
                });
