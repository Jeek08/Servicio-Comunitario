...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($lote && $medicamento && $ml_mg && $vencimiento && $cantidad) {
        $sql = "INSERT INTO ministerio (lote, medicamento, ml_mg, vencimiento, cantidad) 
                VALUES ('$lote', '$medicamento', '$ml_mg', '$vencimiento', '$cantidad')";

        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Nuevo registro creado con éxito'); window.location.href='ministerio.php';</script>";
        } else {
