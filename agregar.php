...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($lote && $medicamento && $ml_mg && $vencimiento && $cantidad) {
        $sql = "INSERT INTO seguro_social (lote, medicamento, ml_mg, vencimiento, cantidad) 
                VALUES ('$lote', '$medicamento', '$ml_mg', '$vencimiento', '$cantidad')";

        if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Nuevo registro creado con éxito'); window.location.href='ssocial.php';</script>";
        } else {
